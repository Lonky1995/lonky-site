...
export default function MarketDashboard({ initialTab = "overview" }: { initialTab?: "overview" | "thesis" }) {
  const [tab, setTab] = useState<"overview" | "thesis">(initialTab);
  const selectTab = (next: "overview" | "thesis") => {
    setTab(next);
    const url = new URL(window.location.href);
    if (next === "thesis") url.searchParams.set("tab", "thesis");
    ...
    <div className="pf-page">
      <header className="pf-header" data-reveal>
        <div>
          ...
          <h1>Market.</h1>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted">公开快照与研究 Thesis 分开呈现。数据的更新时间和证据边界优先于结论。</p>
        </div>
      </header>
      <div className="mt-8 flex gap-6 border-b border-white/10"><button onClick={() => selectTab("overview")} className={`pb-3 text-sm ${tab === "overview" ? "border-b-2 border-white text-white" : "text-muted"}`}>概览</button><button onClick={() => selectTab("thesis")} className={`pb-3 text-sm ${tab === "thesis" ? "border-b-2 border-white text-white" : "text-muted"}`}>Thesis <span className="ml-1 text-xs text-muted">研究记录</span></button></div>
