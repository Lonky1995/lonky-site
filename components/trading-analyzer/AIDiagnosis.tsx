              ...
              return (
                <div key={i}>
                  ...
                  <AnimatePresence>
                    ...
                    {isExpanded && !match && (
                      <div className="mt-2 rounded-lg border border-border bg-background px-4 py-3 text-xs text-muted">
                        未能匹配到对应交易记录
                      </div>
                    )}
                  </AnimatePresence>
        ...
        <div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {diagnosis.tradingRules.map((rule, i) => (
              <div
                key={i}
                className="rounded-lg border border-accent/20 bg-accent/5 px-3 py-2 text-sm text-foreground/80"
              >
                {rule}
              </div>
            ))}
